export type Category =
  | "Macaquinho"
  | "Macacão"
  | "Top"
  | "Camiseta"
  | "Short"
  | "Bermuda"
  | "Calça"
  | "Legging"
  | "Jaqueta"
  | "Conjunto"
  | "Acessório";
export type Gender = "feminino" | "masculino";

export interface Product {
  id: number;
  name: string;
  price: string;
  priceValue: number;
  images: string[];
  description?: string;
  category: Category;
  gender: Gender;
  sizes?: string[];
  highlight?: boolean;
}

export const categories: { label: string; value: Category | "Todos" }[] = [
  { label: "Todos", value: "Todos" },
  { label: "Macaquinhos", value: "Macaquinho" },
  { label: "Macacões", value: "Macacão" },
  { label: "Tops", value: "Top" },
  { label: "Camisetas", value: "Camiseta" },
  { label: "Shorts", value: "Short" },
  { label: "Bermudas", value: "Bermuda" },
  { label: "Calças", value: "Calça" },
  { label: "Leggings", value: "Legging" },
  { label: "Jaquetas", value: "Jaqueta" },
  { label: "Conjuntos", value: "Conjunto" },
  { label: "Acessórios", value: "Acessório" },
];

export const products: Product[] = [
  {
    id: 1,
    name: "Macaquinho Frente Única",
    price: "R$ 351,90",
    priceValue: 351.9,
    images: ["/products/01-frente.jpg", "/products/02-costas.jpg"],
    description: "Macaquinho com design frente única, perfeito para treinos intensos",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 2,
    name: "Macacão Regata c/ Zíper",
    price: "R$ 516,90",
    priceValue: 516.9,
    images: ["/products/03-frente.jpg", "/products/04-costas.jpg"],
    description: "Macacão regata com zíper frontal, ideal para musculação",
    category: "Macacão",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 3,
    name: "Macaquinho Bolso Pocket",
    price: "R$ 375,90",
    priceValue: 375.9,
    images: ["/products/05-frente.jpg", "/products/06-costas.jpg"],
    description: "Macaquinho com bolsos laterais, prático e estiloso",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 4,
    name: "Macaquinho Hyper",
    price: "R$ 368,90",
    priceValue: 368.9,
    images: ["/products/07.jpg"],
    description: "Macaquinho Hyper com zíper, versátil para qualquer atividade",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 5,
    name: "Macaquinho Manga Longa",
    price: "R$ 385,90",
    priceValue: 385.9,
    images: ["/products/08.jpg"],
    description: "Macaquinho manga longa, proteção e conforto",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 6,
    name: "Macaquinho My Fit Mood",
    price: "R$ 158,90",
    priceValue: 158.9,
    images: ["/products/09.jpg"],
    description: "Macaquinho leve e confortável para o dia a dia fitness",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
    highlight: true,
  },
  {
    id: 7,
    name: "Top Verde",
    price: "R$ 152,90",
    priceValue: 152.9,
    images: ["/products/10.jpg", "/products/33.jpg"],
    description: "Top esportivo verde com sustentação e estilo",
    category: "Top",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 8,
    name: "Short Dusell",
    price: "R$ 191,90",
    priceValue: 191.9,
    images: ["/products/11.jpg", "/products/34.jpg"],
    description: "Short Dusell com bolso lateral para celular",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 9,
    name: "Legging Dusell",
    price: "R$ 285,90",
    priceValue: 285.9,
    images: ["/products/12.jpg", "/products/35.jpg"],
    description: "Legging com bolso e tecido brilhoso de alta compressão",
    category: "Legging",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 10,
    name: "Short Sobreposto Valley",
    price: "R$ 262,90",
    priceValue: 262.9,
    images: ["/products/13.jpg", "/products/36.jpg"],
    description: "Short sobreposto Valley, estilo e liberdade de movimento",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 11,
    name: "Conjunto Top + Short Rosa",
    price: "Top R$ 189,90 / Short R$ 191,90",
    priceValue: 380.8,
    images: ["/products/14.jpg"],
    description: "Conjunto combinando top e short na cor rosa",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 12,
    name: "Macacão Preto",
    price: "R$ 339,90",
    priceValue: 339.9,
    images: ["/products/15.jpg", "/products/37.jpg"],
    description: "Macacão preto clássico, elegância para seus treinos",
    category: "Macacão",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 13,
    name: "Short Bolsos Pocket",
    price: "R$ 222,90",
    priceValue: 222.9,
    images: ["/products/16.jpg"],
    description: "Short feminino com bolsos pocket",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 14,
    name: "Top Dusell",
    price: "R$ 260,90",
    priceValue: 260.9,
    images: ["/products/17.jpg"],
    description: "Top Dusell com zíper frontal e mangotes",
    category: "Top",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 15,
    name: "Legging Rosa Imperial",
    price: "R$ 316,90",
    priceValue: 316.9,
    images: ["/products/18.jpg", "/products/41.jpg"],
    description: "Legging rosa imperial Drop Chantilly, alta compressão",
    category: "Legging",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
    highlight: true,
  },
  {
    id: 16,
    name: "Top Rosa Imperial (cor 1)",
    price: "R$ 219,90",
    priceValue: 219.9,
    images: ["/products/19.jpg"],
    description: "Top Rosa Imperial com alça frente única",
    category: "Top",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 17,
    name: "Top Rosa Imperial (cor 2)",
    price: "R$ 210,90",
    priceValue: 210.9,
    images: ["/products/20.jpg"],
    description: "Top Rosa Imperial com alças finas",
    category: "Top",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 18,
    name: "Short Wonder c/ Bolso",
    price: "R$ 191,90",
    priceValue: 191.9,
    images: ["/products/21.jpg"],
    description: "Short Wonder com bolso lateral",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 19,
    name: "Bermuda Moletom Crown",
    price: "R$ 95,90",
    priceValue: 95.9,
    images: ["/products/22.jpg"],
    description: "Bermuda moletom Crown",
    category: "Bermuda",
    gender: "masculino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 20,
    name: "Conjunto de Calça Rosa",
    price: "R$ 200,90",
    priceValue: 200.9,
    images: ["/products/23.jpg"],
    description: "Conjunto de calça rosa fitness",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 21,
    name: "Macaquinho Elite",
    price: "R$ 158,90",
    priceValue: 158.9,
    images: ["/products/24.jpg"],
    description: "Macaquinho Elite feminino, leve e confortável",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 22,
    name: "Conjunto de Calça Lilás",
    price: "R$ 202,90",
    priceValue: 202.9,
    images: ["/products/25.jpg"],
    description: "Conjunto de calça lilás fitness",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 23,
    name: "Conjunto Top + Short Authentic",
    price: "Top R$ 135,90 / Short R$ 119,90",
    priceValue: 255.8,
    images: ["/products/26.jpg"],
    description: "Conjunto Top Authentic e Short Authentic",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 24,
    name: "Conjunto Hot",
    price: "R$ 220,90",
    priceValue: 220.9,
    images: ["/products/27.jpg"],
    description: "Conjunto Hot fitness",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 25,
    name: "Jaqueta Manga Longa",
    price: "R$ 248,90",
    priceValue: 248.9,
    images: ["/products/29.jpg"],
    description: "Jaqueta manga longa fitness, conforto e proteção",
    category: "Jaqueta",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 26,
    name: "Macaquinho Hyper (variação)",
    price: "R$ 368,90",
    priceValue: 368.9,
    images: ["/products/30.jpg"],
    description: "Macaquinho Hyper feminino com zíper",
    category: "Macaquinho",
    gender: "feminino",
    sizes: ["P", "M", "G"],
    highlight: true,
  },
  {
    id: 27,
    name: "Conjunto Top Faixa + Short Hyper",
    price: "Top R$ 133,90 / Short R$ 140,90",
    priceValue: 274.8,
    images: ["/products/31.jpg"],
    description: "Conjunto Top Faixa e Short Hyper Tranpass",
    category: "Conjunto",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 28,
    name: "Camiseta Gota",
    price: "R$ 173,00",
    priceValue: 173,
    images: ["/products/32.jpg"],
    description: "Camiseta Gota feminina",
    category: "Camiseta",
    gender: "feminino",
    sizes: ["P", "M", "G", "GG"],
  },
  {
    id: 29,
    name: "Short Hyper",
    price: "R$ 175,90",
    priceValue: 175.9,
    images: ["/products/38.jpg"],
    description: "Short Hyper feminino com bolso lateral",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 30,
    name: "Short Saia Compression",
    price: "R$ 224,90",
    priceValue: 224.9,
    images: ["/products/39.jpg"],
    description: "Short saia Compression com sobreposição",
    category: "Short",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 31,
    name: "Top Faixa",
    price: "R$ 133,90",
    priceValue: 133.9,
    images: ["/products/40.jpg"],
    description: "Top Faixa feminino",
    category: "Top",
    gender: "feminino",
    sizes: ["P", "M", "G"],
  },
  {
    id: 32,
    name: "Calça Moletom Crown",
    price: "R$ 140,99",
    priceValue: 140.99,
    images: [
      "/products/calca-crown-1.jpg",
      "/products/calca-crown-2.jpg",
      "/products/calca-crown-3.jpg",
      "/products/calca-crown-4.jpg",
      "/products/calca-crown-5.jpg",
      "/products/calca-crown-6.jpg",
      "/products/calca-crown-7.jpg",
      "/products/calca-crown-8.jpg",
      "/products/calca-crown-9.jpg",
      "/products/calca-crown-10.jpg",
    ],
    description: "Calça moletom Crown masculina",
    category: "Calça",
    gender: "masculino",
    sizes: ["P", "M", "G", "GG"],
    highlight: true,
  },
];
